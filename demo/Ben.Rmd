...
ds = # Will take a loooong time -- the first time
...
teams = ddply(ds, ~ batterPos, summarise, G = length(unique(gameId)), PA = sum(isPA), R = sum(runsOnPlay), H = sum(isHit), HR = sum(event == "Home Run"), K = sum(event %in% c("Strikeout", "Strikeout - DP")), OBP = sum(isHit | event %in% c("Walk", "Intent Walk", "Hit By Pitch")) / sum(isPA & !event %in% c("Sac Bunt", "Sacrifice Bunt DP")))
...
Who fielded each ball?

```{r}
ds = transform(ds, wasFielded = !is.na(fielderId))
ds$fielderPos = ifelse(is.na(ds$fielderId), "Hit", "Out")
ds$fielderPos = ifelse(!is.na(ds$fielderId) & ds$fielderId == ds$pitcherId, "P", ds$fielderPos)
ds$fielderPos = ifelse(!is.na(ds$fielderId) & ds$fielderId == ds$playerId.C, "C", ds$fielderPos)
ds$fielderPos = ifelse(!is.na(ds$fielderId) & ds$fielderId == ds$playerId.1B, "1B", ds$fielderPos)
ds$fielderPos = ifelse(!is.na(ds$fielderId) & ds$fielderId == ds$playerId.2B, "2B", ds$fielderPos)
ds$fielderPos = ifelse(!is.na(ds$fielderId) & ds$fielderId == ds$playerId.3B, "3B", ds$fielderPos)
ds$fielderPos = ifelse(!is.na(ds$fielderId) & ds$fielderId == ds$playerId.SS, "SS", ds$fielderPos)
ds$fielderPos = ifelse(!is.na(ds$fielderId) & ds$fielderId == ds$playerId.LF, "LF", ds$fielderPos)
ds$fielderPos = ifelse(!is.na(ds$fielderId) & ds$fielderId == ds$playerId.CF, "CF", ds$fielderPos)
ds$fielderPos = ifelse(!is.na(ds$fielderId) & ds$fielderId == ds$playerId.RF, "RF", ds$fielderPos)
...
Next, we'll build 9 separate models -- each giving the probability of a fielder making a play. 
...
plotFun(fit.2B(x, y) ~ x + y, surface=TRUE, alpha = 1, x.lim = c(-300, 300), y.lim = c(0, 400))
...
What is the probability of **any** fielder making a play? 

```{r}
fit.all = makeFun(fit.P(x, y) + fit.C(x, y) + fit.1B(x, y) + fit.2B(x, y) + fit.3B(x, y) + fit.SS(x,y) + fit.LF(x, y) + fit.CF(x, y) + fit.RF(x,y) ~ x + y)
one = makeFun(1 ~ x + y)
fit.total = makeFun(min(one(x,y), fit.all(x,y)) ~ x + y)
plotFun(fit.total(x, y) ~ x + y, surface=TRUE, alpha = 1, x.lim = c(-300, 300), y.lim = c(0, 500))
```
...
```{r}
war.P = ddply(ds, ~pitcherId, summarise, RAA.P = sum(delta.P, na.rm=TRUE))
...
```{r}
war.C = ddply(ds, ~playerId.C, summarise, RAA.C = sum(delta.C, na.rm=TRUE))
war.C[order(war.C$RAA.C, decreasing=FALSE),]
densityplot(~RAA.C, data=war.C)
```

```{r}
war.1B = ddply(ds, ~playerId.1B, summarise, RAA.1B = sum(delta.1B, na.rm=TRUE))
war.2B = ddply(ds, ~playerId.2B, summarise, RAA.2B = sum(delta.2B, na.rm=TRUE))
war.3B = ddply(ds, ~playerId.3B, summarise, RAA.3B = sum(delta.3B, na.rm=TRUE))
war.SS = ddply(ds, ~playerId.SS, summarise, RAA.SS = sum(delta.SS, na.rm=TRUE))
war.LF = ddply(ds, ~playerId.LF, summarise, RAA.LF = sum(delta.LF, na.rm=TRUE))
war.CF = ddply(ds, ~playerId.CF, summarise, RAA.CF = sum(delta.CF, na.rm=TRUE))
war.RF = ddply(ds, ~playerId.RF, summarise, RAA.RF = sum(delta.RF, na.rm=TRUE))
densityplot(~RAA.RF, data=war.RF)
```

The distribution is not fair, because balls hit to the outfield have higher run consequence.
...
head(dw[order(dw$delta.CF, decreasing=TRUE), c("event", "startCode", "endCode", "startOuts", "endOuts", "runsOnPlay", "startExR", "endExR", "delta", "delta.pitch", "delta.field", "delta.CF", "description")], 100)
```
...
```{r}
head(players[order(players$RAA, decreasing=TRUE), c("Name", "RAA", "RAA.bat", "RAA.br", "RAA.field", "RAA.pitch")], 100)
head(players[order(players$RAA, decreasing=FALSE), c("Name", "RAA", "RAA.bat", "RAA.br", "RAA.field", "RAA.pitch")], 100)
```
