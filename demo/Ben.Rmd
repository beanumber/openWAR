...
teams = ddply(ds, ~ startPos, summarise, G = length(unique(gameId)), PA = sum(isPA), R = sum(runsOnPlay), H = sum(isHit), HR = sum(event == "Home Run"), K = sum(event %in% c("Strikeout", "Strikeout - DP")), OBP = sum(isHit | event %in% c("Walk", "Intent Walk", "Hit By Pitch")) / sum(isPA & !event %in% c("Sac Bunt", "Sacrifice Bunt DP")))
...
```{r}
mod.bat = lm(delta ~ event + startPos + stadium + (stand == throws), data=ds)
summary(mod.bat)
```
...
```{r}
war.off = merge(x=war.bat, y=war.br1, by.x="batterId", by.y="start1B", all.x=TRUE)
war.off = merge(x=war.off, y=war.br2, by.x="batterId", by.y="start2B", all.x=TRUE)
war.off = merge(x=war.off, y=war.br3, by.x="batterId", by.y="start3B", all.x=TRUE)
war.off[is.na(war.off)] = 0
war.off = transform(war.off, RAA.br = RAA.br1 + RAA.br2 + RAA.br3)
war.off = transform(war.off, RAA = RAA.bat + RAA.br)
war.off[order(war.off$RAA, decreasing=TRUE),]
```
